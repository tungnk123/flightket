...
{
    public partial class FormTraCuuChuyenBayQuanLi : Form
    {
        public static FormTraCuuChuyenBayQuanLi instance;
        public string maChuyenBay;
        public string sanBayDi;
        public string sanBayDen;
        public DateTime ngayKhoiHanh;
        FlightKetDBEntities db;
        public FormTraCuuChuyenBayQuanLi()
        {
            instance = this;
            InitializeComponent();
            dp_ngayKhoiHanh.Value = DateTime.Now;
            db = new FlightKetDBEntities();
            ...
            sanBayDen = cb_sanBayDen.Text;
                ...
                if (resultList.Count > 0)
                {
                    ...
                    for (int i = 0; i < resultList.Count; i++)
                    {
                        ...
                        dgv_chuyenBayPhuHop.Rows[i].Cells[8].Value = "Xóa";

                        DataGridViewCellStyle dataGridViewCellStyle = new DataGridViewCellStyle();
                        dataGridViewCellStyle.ForeColor = Color.White;
                        dataGridViewCellStyle.BackColor = Color.SkyBlue;
                        dataGridViewCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
                        dgv_chuyenBayPhuHop.Rows[i].Cells[8].Style = dataGridViewCellStyle;
                    }
        ...
        {
            string maChuyenBay = dgv_chuyenBayPhuHop.SelectedCells[0].OwningRow.Cells[1].Value.ToString();
            
                CHUYENBAY cHUYENBAY = db.CHUYENBAYs.Find(maChuyenBay);
                // xu ly phieu dat cho
                var result1 = db.PHIEUDATCHOes.Where(a => a.MaChuyenBay == cHUYENBAY.MaChuyenBay).ToList();
                var result2 = db.VECHUYENBAYs.Where(a => a.MaChuyenBay == cHUYENBAY.MaChuyenBay).ToList();
            try
            {
                if (result1.Count > 0 || result2.Count > 0)
                {
                    MessageBox.Show("Không thể xóa chuyến bay này vì nó đã có người đặt");
                    return;
                }
                else
                {
                    var result4 = db.CT_HANGVE.Where(a => a.MaChuyenBay == cHUYENBAY.MaChuyenBay).ToList();
                    foreach (var item in result4)
                    {
                        db.CT_HANGVE.Remove(item);
                        db.SaveChanges();
                    }
                    var result5 = db.CT_CHUYENBAY.Where(a => a.MaChuyenBay == cHUYENBAY.MaChuyenBay).ToList();
                    foreach (var item1 in result5)
                    {
                        db.CT_CHUYENBAY.Remove(item1);
                        db.SaveChanges();
                    }
            ...
            {
                MessageBox.Show("Không thể xóa chuyến bay này vì nó đã được đặt chỗ");
                return;
            }
